...
class SentenceExplanationService:
    ...
    def _build_prompt(
        ...
        return dedent(
            f"""
            你是一位專業的英語導讀老師，需要用{language}（繁體中文）為學習者解釋句子。
            ...
            請以 JSON 格式回答，包含：
            {{
              "overview": "用 1-2 句描述句子重點與情境",
              "key_points": ["重點說明，最多 4 項"],
              "vocabulary": [
                {{"word": "重要單字或片語", "meaning": "中文解釋", "note": "補充說明，可選"}}
              ]
            }}
            ...
            - 使用繁體中文
            - 保持 JSON 合法且鍵名固定
            - vocabulary 中的 word 與 meaning 為必填，note 可省略或為空字串
            """
        ).strip()
